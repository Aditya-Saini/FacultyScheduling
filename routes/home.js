						...
						if(err)
							...
							User.find({}, (err, foundUsers)=>{
								if(err)
									...
									}).sort({ start: 1 }).then(foundTask=>{
										res.render("calendar",{event:foundEvent.events, userid:req.user._id, people:foundUsers,search: freeUsers, tasks:foundTask, moment:moment, messages:req.flash("success")});
			...
			User.find({}, (err, foundUsers)=>{
				if(err)
					...
					Task.find( //query today up to tonight
					{
						...
						res.render("calendar",{event:foundEvent.events, userid:req.user._id, people:foundUsers,search: freeUsers, tasks:foundTasks, moment:moment, messages:req.flash("success")});
